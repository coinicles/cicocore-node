...
var bitcore = require('cicocore-lib');
var zmq = require('zmq');
var async = require('async');
var LRU = require('lru-cache');
var CicoRPC = require('cicod-rpc');
...
/**
 * Provides a friendly event driven API to cicod in Node.js. Manages starting and
 * stopping cicod as a child process for application support, as well as connecting
 * to multiple cicod processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function Cico(options) {
  if (!(this instanceof Cico)) {
    return new Cico(options);
  }
...
util.inherits(Cico, Service);

Cico.dependencies = [];

Cico.DEFAULT_MAX_TXIDS = 1000;
Cico.DEFAULT_MAX_HISTORY = 50;
Cico.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Cico.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Cico.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Cico.DEFAULT_SPAWN_RESTART_TIME = 5000;
Cico.DEFAULT_SPAWN_STOP_TIME = 10000;
Cico.DEFAULT_TRY_ALL_INTERVAL = 1000;
Cico.DEFAULT_REINDEX_INTERVAL = 10000;
Cico.DEFAULT_START_RETRY_INTERVAL = 5000;
Cico.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Cico.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Cico.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  rpcuser: 'cico',
  rpcpassword: 'cicopassword',
  uacomment: 'bitcore'
};

Cico.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Cico.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Cico.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || Cico.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Cico.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Cico.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Cico.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Cico.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Cico.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Cico.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Cico.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Cico.prototype._initCaches = function() {
...
Cico.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Cico.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
Cico.prototype.getPublishEvents = function() {
  return [
    {
...
Cico.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'cicod/' + name, 'total:', this.subscriptions[name].length);
};

Cico.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'cicod/' + name, 'total:', this.subscriptions[name].length);
};


Cico.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
...
Cico.prototype.unsubscribeBalance = function(emitter, addresses) {
...
Cico.prototype.unsubscribeBalanceAll = function(emitter) {
...
Cico.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'cicod/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Cico.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'cicod/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Cico.prototype.unsubscribeAddressAll = function(emitter) {
...
Cico.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Cico.DEFAULT_CONFIG_SETTINGS;
...
Cico.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
Cico.prototype._expandRelativeDatadir = function() {
...
Cico.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in cicod config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in cicod config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in cicod config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './cico.conf');

  log.info('Using cico config file:', configPath);
...
Cico.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of cicocore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of cicocore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of cicocore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to cicod from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from cicod. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from cicod. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that cicod is undergoing a reindex. ' +
             ...
             'of cicocore-node services will start.');
    node._reindex = true;
  }
};

Cico.prototype._resetCaches = function() {
...
Cico.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Cico.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Cico.prototype._initChain = function(callback) {
  var self = this;
...
Cico.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 18332
  };
...
Cico.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/cico.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/cico.conf';
    }
  }
  return networkPath;
};

Cico.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
Cico.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('cicod/hashblock', message.toString('hex'));
    }
  }

};

Cico.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Cico.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Cico Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Cico.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Cico.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
Cico.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
...
Cico.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('cicod/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Cico.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('cicod/rawtransaction', message.toString('hex'));
    }
...
Cico.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Cico.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Cico.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Cico.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
Cico.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('Cico Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || Cico.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

Cico.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Cico.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/cicod.pid';
...
Cico.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting cico process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Cico process unexpectedly exited with code:', code);
        log.warn('Restarting cico child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Cico process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn cicod.'));
      }
...
Cico.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to cicod.'));
    }
 ...
 */
Cico.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Cico configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Cico.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Cico.prototype.syncPercentage = function(callback) {
  var self = this;
...
Cico.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, cicocore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Cico.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
Cico.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Cico.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Cico.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in cicocore is most recent to less recent
 ...
 * @param {Function} callback
 */
Cico.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
Cico.prototype._getConfirmationsDetail = function(transaction) {
...
Cico.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Cico.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Cico.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A cicocoin transaction id
 * @param {Function} callback
 */
Cico.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Cico.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
Cico.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
Cico.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Cico.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Cico.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
Cico.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Cico.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
Cico.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
Cico.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Cicocore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Cico.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getInfo = function(callback) {
...
Cico.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('cicod spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('cicod process did not exit'));
      }
 ...
 * @param {Function} callback
 */
Cico.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
Cico.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
Cico.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
Cico.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
Cico.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
Cico.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
Cico.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
Cico.prototype.getStakingInfo = function(callback) {
    var self = this,

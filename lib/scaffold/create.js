...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'cicocore-lib': 'https://github.com/coinicles/cicocore-lib.git#master',
    'cicocore-node': 'https://github.com/coinicles/cicocore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/cicocore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the cicocore-node directory and configuration
